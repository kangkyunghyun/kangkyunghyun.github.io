...
결론부터 쓰면 쓸 만하지 않았다. 개발 서버 로그의 92%가 같은 경고 한 종류였고 애초에 찍힐 이유가 없는 것이었다. 이 글은 인프라를 세우기 전에 로그 소스부터 고친 기록이다.
...
// 비즈니스 API(/api/*) 밖의 경로는 헬스체크(ALB, ECS)와 취약점 스캐너 등 기계 트래픽이라
