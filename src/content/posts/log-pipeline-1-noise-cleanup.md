...
결론부터 쓰면 쓸 만하지 않았다. 개발 서버 로그의 92%가 같은 경고 한 종류였다. 그 경고는 애초에 찍힐 이유가 없는 것이었다. 이 글은 인프라를 세우기 전에 로그 소스부터 고친 기록이다.
...
문제는 대상이다. ALB 헬스체커는 우리 앱의 헤더 규약을 알 리가 없다. 취약점 스캐너도 마찬가지다. 이런 기계 트래픽은 헤더가 없는 게 **정상**이다. 정상 상황에 경고를 찍고 있었다. 그게 로그의 92%를 차지했다.
...
**경고만 끈다**는 점이 중요하다. `request_id` 발급과 MDC 적재는 경로와 무관하게 그대로 돈다. 헬스체크 요청도 추적 자체는 되어야 한다. 다만 "헤더가 없다"고 떠들 필요가 없을 뿐이다.
...
다음 글에서는 이 로그를 담을 OpenSearch 인덱스 템플릿을 만든다. `status_code`를 문자열로 잡으면 왜 곤란해지는지와 `keyword`·`text`를 나누는 기준이 무엇인지를 실제 쿼리로 확인한다.
