...
```text
manyak-server ──(1) 컨테이너 왕복 + AI 서버 처리 ──▶ manyak-ai ──(2) LLM ──▶ 제공자
              └───── manyak.ai.call.duration ───────┘  └── Langfuse ────┘
```

Micrometer는 (1)과 (2)를 합쳐 재고 Langfuse는 (2)만 잰다. 그 사이에 프롬프트 조립, 응답 검증, 재시도, 컨테이너 간 네트워크가 있고 서버와 AI가 같은 인스턴스에 얹혀 있으니 자원 경합도 여기 들어간다. Langfuse만 보면 LLM은 3초인데 사용자는 12초를 기다린 상황을 잡지 못한다.
