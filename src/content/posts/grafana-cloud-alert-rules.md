...
멘토링에서 받은 조언도 같은 방향이었다. 지표를 잘게 나누는 것보다 AI 서비스가 죽는 것을 가장 강하게 경고해야 한다는 이야기, 그리고 지금 단계에서는 특정 시간 동안 4xx와 5xx가 몇 건 이상이면 보내는 정도로 단순하게 만들고 실제로 알림이 오는지부터 확인하라는 이야기였다. 복잡한 규칙은 서비스가 잘 돌아가기 시작한 다음에 필요하고 그전에 넣으면 알림 폭탄만 된다는 말도 함께 들었다.
...
이번 수정은 서버 코드 변경이라 릴리스가 필요했다. 운영 이미지는 `main` 기준으로 빌드되므로 `dev` 머지만으로는 카운터가 사전 등록되지 않는다. 이전 글에서 두 번 겪은 함정이라 이번에는 릴리스 티켓 설명에 미리 적어 뒀다.
