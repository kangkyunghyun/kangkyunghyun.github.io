...
OpenSearch는 스키마를 미리 정하지 않아도 쓸 수 있다. 문서를 넣으면 알아서 타입을 추측한다. 편하지만 사고가 난다. 이 글은 인덱스 템플릿을 왜 미리 만들어야 하는지, 그리고 `keyword`와 `text`를 나누는 기준이 무엇인지 실제 쿼리로 확인한 기록이다.
...
여기서 용어를 정리하고 넘어가는 게 좋다. 이름이 비슷한 물건이 둘이고, **사는 곳이 다르다.**
