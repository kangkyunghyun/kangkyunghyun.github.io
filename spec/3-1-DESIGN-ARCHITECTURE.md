...
| --- | --- | --- | --- |
| `--fg` | `#191f28` | `#f9fafb` | 본문 |
| `--fg2` | `#4e5968` | `#ced4da` | 보조 텍스트 |
| `--fg3` | `#667080` | `#aab3bf` | 날짜, 라벨, 보조 문구 |
| `--bg` | `#ffffff` | `#17171c` | 배경 |
...
회색과 파랑은 **면에 쓰는 값과 글자에 쓰는 값이 다르다.** 라이트 모드에서 `#3182f6`은 흰 배경에서 3.6:1, `#8b95a1`은 2.9:1로 둘 다 본문 기준(4.5:1)에 못 미친다. 눈에는 예뻐 보여도 기준 미달이므로 글자에는 `--blue-strong`·`#667080`을 쓴다. 다크 모드는 배경이 어두워 같은 문제가 없어 값이 갈린다. `MUST`

**다크 모드는 4.5:1로 부족하다.** 어두운 배경에서는 같은 비율이라도 더 흐리게 읽힌다. 13~15px 한글 보조 텍스트가 5.3:1일 때 "안 보인다"는 지적이 실제로 나왔다. 다크의 회색은 **7:1 이상**을 목표로 잡는다. `SHOULD`

이때 `--fg3`만 올리면 `--fg2`와 붙어 위계가 무너지므로 **둘을 함께 올려 간격을 유지한다.** 현재 다크의 단계는 본문 15:1 · 보조 10.7:1 · 흐린 글 7.6:1이다.

코드 하이라이팅도 같은 이유로 라이트 테마를 `github-light-high-contrast`로 둔다. 기본 `github-light`는 일부 토큰이 카드 면 위에서 4.4:1까지 떨어진다.
...
| 글 본문 | 16px / 줄간격 1.7 |
| 카드 설명, 히어로 소개 | 16px |
| 보조 텍스트 (기간·역할·라벨·칩) | **14px** |
| 장식 기호(`↗`) | 12px |

**보조 텍스트를 13px 아래로 내리지 않는다.** 한글은 획이 많아 같은 px에서 라틴 문자보다 작게 읽히고, 이 값들은 대부분 흐린 회색(`--fg3`)이라 더 눌려 보인다. 실제로 13px로 두었을 때 메인 페이지 텍스트의 절반 가까이가 그 크기였고 작다는 지적을 받았다. `SHOULD`

목차(`Toc.astro`)만 13px를 유지한다 — 좁은 폭에 항목이 많아 밀도가 우선이다.

## 3-1-5 링크가 열리는 곳

외부 링크는 **UI 요소인지 본문인지**로 갈린다. `MUST`

| 위치 | 처리 |
| --- | --- |
| 히어로 버튼, 푸터 Connect, 타임라인, 프로젝트 상세 헤더 링크, 안내 문구 | 새 탭 (`target="_blank"` + `rel="noopener noreferrer"` + `↗` 표시) |
| 글·프로젝트 **마크다운 본문** 안의 링크 | 같은 탭 |
| `mailto:` | 같은 탭 — 새 탭을 열면 메일 앱이 뜨고 빈 탭이 남는다 |
| 내부 경로(`/blog`, `/#projects`) | 같은 탭 |

새 탭으로 여는 링크에는 **반드시 `↗` 같은 표시를 붙인다.** 예고 없이 맥락이 바뀌면 스크린리더 사용자는 뒤로가기가 듣지 않는 이유를 알 수 없다. `rel`은 새 창이 `window.opener`로 원래 탭을 조작하는 것을 막는다.

본문 링크까지 새 탭으로 열면 글을 읽는 동안 탭이 계속 쌓인다. 그래서 본문은 제외한다.
